...
class MhsPklResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        $tanggalAwal = Carbon::parse($this->r_usulan->tgl_awal_pkl)->format('M d, Y');
        $tanggalAkhir = Carbon::parse($this->r_usulan->tgl_akhir_pkl)->format('M d, Y');
        ...
        if ($this->r_usulan->tgl_awal_pkl > Carbon::now()) {
            $status_pkl = 'Belum Magang';
        } elseif ($this->r_usulan->tgl_awal_pkl <= Carbon::now() && $this->r_usulan->tgl_akhir_pkl >= Carbon::now()) {
        ...
        //Nilai
        $nilai_pembimbing = PklNilai::where('dosen_id', $this->pembimbing_id)->where('pkl_mhs_id', $this->id_pkl_mhs)->where('sebagai', 'pembimbing')->select('nilai')->first();
        $nilai_penguji_1 = PklNilai::where('dosen_id', $this->pembimbing_id)->where('pkl_mhs_id', $this->id_pkl_mhs)->where('sebagai', 'penguji')->select('nilai')->first();
        $nilai_penguji_2 = PklNilai::where('dosen_id', $this->penguji_id)->where('pkl_mhs_id', $this->id_pkl_mhs)->where('sebagai', 'penguji')->select('nilai')->first();
        return [
            ...
            'tempat_pkl' => $this->r_usulan->r_tempat_pkl->nama_tempat_pkl,
            'role_pkl' => $this->r_usulan->r_role_tempat_pkl->nama_role,
            'tgl_awal_pkl' => $tanggalAwal,
            'tgl_akhir_pkl' => $tanggalAkhir,
            'status_usulan' => $this->r_usulan->status_usulan,
            'alamat_perusahaan' => $this->r_usulan->alamat_tempat_pkl,
